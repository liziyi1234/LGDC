...
from UGFE import ERPCoordLatitudeWeight
...
class LGDC(nn.Module):
    def __init__(self):
        ...
        self.aqca_modules = nn.ModuleList([
            AQCA(in_channels=192, out_channels=192, se_reduction=16, dilation_rates=(1, 3, 5)),
            AQCA(in_channels=384, out_channels=384, se_reduction=16, dilation_rates=(1, 3, 5)),
            AQCA(in_channels=768, out_channels=768, se_reduction=16, dilation_rates=(1, 3, 5)),
            AQCA(in_channels=768, out_channels=768, se_reduction=16, dilation_rates=(1, 3, 5))
        ])
        
        self.aqca_adjust_layers = nn.ModuleList([
        ...
        self.ddas_modules = nn.ModuleList([
            DDAS(in_channels=192, kernel_sizes=(3, 5, 7, 9), fusion='learnable'),
            DDAS(in_channels=384, kernel_sizes=(3, 5, 7, 9), fusion='learnable'),
            DDAS(in_channels=768, kernel_sizes=(3, 5, 7, 9), fusion='learnable'),
            DDAS(in_channels=768, kernel_sizes=(3, 5, 7, 9), fusion='learnable')
        ])
        
     
        self.ddas_adjust_layers = nn.ModuleList([
            nn.Conv2d(192, 256, kernel_size=1),
            nn.Conv2d(384, 256, kernel_size=1),
            nn.Conv2d(768, 256, kernel_size=1),
            nn.Conv2d(768, 256, kernel_size=1)
        ...
        self.mrff_fusion_modules = nn.ModuleList([
            MRFF(
                dim=256, 
                large_kernel=9, 
                small_kernel=3, 
    ...
    def feature_forward(self, x):
        ...
        for i, stage_out in enumerate(stage_outputs):
            ...
            aqca_out = self.aqca_modules[i](stage_bchw)  # [B, C, H, W]，保持输入维度
            aqca_out_adjusted = self.aqca_adjust_layers[i](aqca_out)  # [B, 256, H, W]
            aqca_features.append(aqca_out_adjusted)
            
    
            ddas_out = self.ddas_modules[i](stage_bchw)
            ddas_out_adjusted = self.ddas_adjust_layers[i](ddas_out)  # [B, 256, H, W]
            ddas_features.append(ddas_out_adjusted)
        
  
        mrff_outputs = []
        for i in range(4):
            mrff_fused = self.mrff_fusion_modules[i](aqca_features[i], ddas_features[i])  # [B, 256, H, W]
            # 上采样到统一尺寸 (128x128)
            mrff_fused_upsampled = self.upsample_layers[i](mrff_fused)
            mrff_outputs.append(mrff_fused_upsampled)
        
        # 将四个融合输出相加 (现在都是 [B, 256, 128, 128])
        final_feature = torch.zeros_like(mrff_outputs[0])
        for mrff_out in mrff_outputs:
            final_feature += mrff_out
