...
class LGDC(nn.Module):
    def __init__(self):
        ...
        self.acdp_modules = nn.ModuleList([
            ACDP(in_channels=192, kernel_sizes=(3, 5, 7, 9), fusion='learnable'),
            ACDP(in_channels=384, kernel_sizes=(3, 5, 7, 9), fusion='learnable'),
            ACDP(in_channels=768, kernel_sizes=(3, 5, 7, 9), fusion='learnable'),
            ACDP(in_channels=768, kernel_sizes=(3, 5, 7, 9), fusion='learnable')
        ])
        
     
        self.acdp_adjust_layers = nn.ModuleList([
            nn.Conv2d(192, 256, kernel_size=1),
            nn.Conv2d(384, 256, kernel_size=1),
            nn.Conv2d(768, 256, kernel_size=1),
            nn.Conv2d(768, 256, kernel_size=1)
    ...
    def feature_forward(self, x):
        ...
        for i, stage_out in enumerate(stage_outputs):
            ...
            acdp_out = self.acdp_modules[i](stage_bchw)
            acdp_out_adjusted = self.acdp_adjust_layers[i](acdp_out)  # [B, 256, H, W]
            acdp_features.append(acdp_out_adjusted)
        
  
        cgdc_outputs = []
        for i in range(4):
            cgdc_fused = self.cgdc_fusion_modules[i](mqca_features[i], acdp_features[i])  # [B, 256, H, W]
            # 上采样到统一尺寸 (128x128)
